...
def generate_dataset(stream: ProductStream, lang: Optional[str]) -> Iterator[JSONType]:
    ...
    for product in stream.iter():
        ...
        inferred_categories_tags: List[TaxonomyNode] = list(infer_category_tags(categories_tags,
                                                                                category_taxonomy))

        if inferred_categories_tags:
            ingredient_tags = product.get('ingredients_tags', [])
            ingredient_tags = [x for x in ingredient_tags if x]
            known_ingredient_tags = [ingredient_tag
                                     for ingredient_tag in ingredient_tags
                                     if ingredient_tag in ingredient_taxonomy]
            ingredients_text_field = 'ingredients_text_{}'.format(lang) if lang else 'ingredients_text'
            ingredients_text = product.get(ingredients_text_field, None) or None

            product_name_field = 'product_name_{}'.format(lang) if lang else 'product_name'
            yield {
                'code': product['code'],
...
def run(lang: Optional[str] = None):
    ...
    if lang is not None:
        training_stream = (training_stream.filter_text_field('lang', lang)
    ...
    count = dump_jsonl(settings.PROJECT_DIR / 'datasets' / 'category' / 'category_{}.jsonl'.format(lang or 'xx'), dataset_iter)
    print(count)
