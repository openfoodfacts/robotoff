...
from typing import Dict, Tuple, List, Optional, AbstractSet

"""
/!\ /!\
WRONG WAY TO INITIALIZE DATASET
TODO : GRACEFULLY DOWNLOAD AND LOAD TOKENS
/!\ /!\
"""
TAXONOMIES_PATH = (
...
class Vocabulary(object):
    ...
    def __init__(
        ...
        if voc_name is not None:
            try:
                path = TAXONOMIES_PATH / self.FILENAMES[voc_name]
                with path.open("r") as f:
                    self.voc = set(
                        self.normalize(token) for token in f.read().split("\n")
                    )
            except FileNotFoundError:
                raise FileNotFoundError("Vocabulary files need to be downloaded.")
