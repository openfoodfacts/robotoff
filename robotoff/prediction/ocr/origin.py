...
class OriginParser:
    ...
    def initialize(self):
        ...
        COUNTRIES_IN_ALL_LANGS = get_taxonomy(TaxonomyType.origin.name)
        self.COUNTRIES_IN_ALL_LANGS = COUNTRIES_IN_ALL_LANGS

        COUNTRIES_FR = [
            country.get_localized_name("fr")
            for country in COUNTRIES_IN_ALL_LANGS.iter_nodes()
            if "fr" in country.names
        ]
        ...
        COUNTRIES_NATIONALITY_ADJECTIVES_FR = [
            country.additional_data["nationalities"]["fr"]
            for country in COUNTRIES_IN_ALL_LANGS.iter_nodes()
            if "fr" in country.additional_data.get("nationalities", {})
        ]
        ...
        COUNTRY_ADJECTIVE_FR_TO_COUNTRY_NAME_FR = {
            COUNTRIES_IN_ALL_LANGS[country.id]["nationalities"]["fr"]: country.id
            for country in COUNTRIES_IN_ALL_LANGS.iter_nodes()
            if "fr"
            in COUNTRIES_IN_ALL_LANGS[country.id].additional_data.get(
                "nationalities", {}
            )
        }
        ...
        COUNTRIES_EN = [
            country.get_localized_name("en")
            for country in COUNTRIES_IN_ALL_LANGS.iter_nodes()
            if "en" in country.names
        ]
        COUNTRIES_EN.extend([r"European Union", r"E\.?U\.?"])
        COUNTRIES_EN_JOINED = "|".join(COUNTRIES_EN)

        COUNTRIES_NATIONALITY_ADJECTIVES_EN = [
            country.additional_data["nationalities"]["en"]
            for country in COUNTRIES_IN_ALL_LANGS.iter_nodes()
            if "en" in country.additional_data.get("nationalities", {})
        ]
        ...
        COUNTRY_ADJECTIVE_EN_TO_COUNTRY_NAME_EN = {
            ...
            if "en" in COUNTRIES_IN_ALL_LANGS[country_id].get("nationalities", {})
        }
    ...
    def find_origin(self, content: Union[OCRResult, str]) -> List[Prediction]:
        ...
        for lang, regex in self.EVERY_REGEX_BY_LANG.items():
            ...
            for match in regex.regex.finditer(text):
                ...
                if origin not in ingredients_origins:
        ...
        if (
            ...
            try:
                return next(  # find the translation in english of the specified country name
                    country.id
                    for country in self.COUNTRIES_IN_ALL_LANGS.iter_nodes()
                    if country.names[lang] == standardized_name
                )
