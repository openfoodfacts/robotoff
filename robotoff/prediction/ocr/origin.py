...
<<<<<<< HEAD
        ...
        self.COUNTRIES_IN_ALL_LANGS = json.load(
            open(settings.OCR_TAXONOMY_COUNTRIES_PATH, "r")
        )

        COUNTRIES_FR = [
            country["name"]["fr"]
            for country in self.COUNTRIES_IN_ALL_LANGS.values()
...
def find_origin(content: Union[OCRResult, str]) -> List[Prediction]:
    ...
    for lang, regex in EVERY_REGEX_BY_LANG.items():
        ...
        if not text:
            return []
>>>>>>> bb64958394fbecd8f7ea7b90a67877a07ad6d59e

        COUNTRY_ADJECTIVE_FR_TO_COUNTRY_NAME_FR = {
            self.COUNTRIES_IN_ALL_LANGS[country_id]["nationalities"]["fr"]: country_id
            for country_id in self.COUNTRIES_IN_ALL_LANGS.keys()
            if country_id in self.COUNTRIES_IN_ALL_LANGS
            and "nationalities" in self.COUNTRIES_IN_ALL_LANGS[country_id]
            and "fr" in self.COUNTRIES_IN_ALL_LANGS[country_id]["nationalities"]
        }
...
<<<<<<< HEAD
        ...
        COUNTRIES_EN = [
            coutry["name"]["en"]
            for coutry in self.COUNTRIES_IN_ALL_LANGS.values()
            if "name" in coutry and "en" in coutry["name"]
        ]
        COUNTRIES_EN.extend([r"European Union", r"E\.?U\.?"])
        COUNTRIES_EN_JOINED = "|".join(COUNTRIES_EN)
        ...
        COUNTRY_ADJECTIVE_EN_TO_COUNTRY_NAME_EN = {
            self.COUNTRIES_IN_ALL_LANGS[country_id]["nationalities"]["en"]: country_id
            for country_id in self.COUNTRIES_IN_ALL_LANGS.keys()
            if country_id in self.COUNTRIES_IN_ALL_LANGS
            and "nationalities" in self.COUNTRIES_IN_ALL_LANGS[country_id]
            and "en" in self.COUNTRIES_IN_ALL_LANGS[country_id]["nationalities"]
        }
    ...
    def extract_origin_from_match(self, origin_match, lang: str) -> str:
        if (
            ...
            try:
                ...
                print(self.COUNTRIES_IN_ALL_LANGS["en:france"]["name"]["fr"])
                return next(  # find the translation in english of the specified country name
                    country_id
