    ...
    for node in taxonomy.iter_nodes():
        for lang in set(node.names) | set(node.synonyms):
            ...
            for name in names:
                ...
                # Don't add it to the store if empty
                if processed:
...
def get_processors() -> Dict[str, KeywordProcessor]:
    ...
    for lang, items in match_maps.items():
        ...
        for pattern, results in items.items():
            processor.add_keyword(pattern, [(id_, pattern) for id_, _ in results])
...
def match(query: str, lang: str) -> List[Tuple[str, str]]:
    ...
    for (id_, pattern), start_idx, _ in flat_results:
        ...
        if id_ in category_ingredient_intersect and start_idx != 0:
            continue
        filtered.append((id_, pattern))
    return filtered


def predict(product: Dict) -> Optional[Prediction]:
    """Predict a product category using a matching algorithm.

    Flashtext always return the longuest match if several pattern match the
    query. Therefore, if more than one category is predicted, it means there
    was two non-overlapping matches: we don't return any prediction in that
    case.
    ...
    for lang in set(product.get("languages_codes", [])) & SUPPORTED_LANG:
        ...
        return Prediction(
            ...
            value_tag=prediction[0],
            data={
                "lang": lang,
                "product_name": product_name,
                "pattern": prediction[1],
            },
