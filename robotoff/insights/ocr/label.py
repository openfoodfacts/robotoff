...
def process_eu_bio_label_code(match) -> str:
    return (
        "en:{}-{}-{}".format(match.group(1), match.group(2), match.group(3))
        .lower()
        .replace("ö", "o")
        .replace("ø", "o")
    )
...
LABELS_REGEX = {
    ...
    "xx-bio-xx": [
        ...
        OCRRegex(
            re.compile(
                r"(?<![a-zA-Z])([A-Z]{2})[\-\s.](BIO|ÖKO|OKO|EKO|ØKO|ORG|ECO|Bio)[\-\s.](\d{2,3})"
            ),
    ...
    "fr:aoc": [
        ...
        OCRRegex(
            re.compile(r"appellation d'origine contr[ôo]l[eé]e"),
            field=OCRField.full_text_contiguous,
            lowercase=True,
        ),
    ],
    ...
    "en:made-in-spain": [
        OCRRegex(
            re.compile(
                r"made in spain|hecho en espa[ñn]a|geproduceerd in spanje|fabriqu[ée] en espagne|hergestellt in spanien"
            ),
            field=OCRField.full_text_contiguous,
            lowercase=True,
        ),
    ...
    "en:gluten-free": [
        OCRRegex(
            re.compile(
                r"(?<!\w)(?:sans|ni) gluten|gluten[- ]free|glutenvrij|senza glutine|sin gluten|glutenfrei|sem gluten|gluténmentes|bez lepku|не містить глютену|bezglutenomy|без глютена"
            ),
            field=OCRField.full_text_contiguous,
            lowercase=True,
        ),
    ...
    "en:vegan": [
        OCRRegex(
            re.compile(r"(?<!\w)(?:vegan|v[ée]g[ée]talien|vegano|veganistisch)(?!\w)"),
            field=OCRField.full_text_contiguous,
            lowercase=True,
        ),
    ],
    ...
    "en:palm-oil-free": [
        OCRRegex(
            re.compile(
                r"без пальмового масла|senza olio di palma|ohne palmöl|(?<!\w)(?:sans|ni) huile de palme|sin aceite de palma|palm oil[ -]free"
            ),
            field=OCRField.full_text_contiguous,
            lowercase=True,
        ),
