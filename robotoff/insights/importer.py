...
from robotoff.models import ProductIngredient, ProductInsight, batch_insert
from robotoff.products import Product, ProductStore
            ...
            for correction in corrections:
                ...
                if key not in insight_seen:
                    ...
                    corrected_snippet = self.generate_snippet(
                        text, start_offset, end_offset, correction["correction"]
                    )
                    insights.append(
                        {
                            "id": str(uuid.uuid4()),
...
class OCRInsightImporter(InsightImporter, metaclass=abc.ABCMeta):
    def import_insights(self, data: Iterable[Dict], automatic: bool = False) -> int:
        ...
        for barcode, insights in grouped_by.items():
            ...
            inserts += list(
                self._process_product_insights(barcode, insights, timestamp, automatic)
            )
    ...
    def _process_product_insights(
        ...
        countries_tags = getattr(self.product_store[barcode], "countries_tags", [])
        brands_tags = getattr(self.product_store[barcode], "brands_tags", [])
    ...
    def process_product_insights(
        self, barcode: str, insights: List[JSONType]
    ) -> Iterable[JSONType]:
        pass
...
class PackagerCodeInsightImporter(OCRInsightImporter):
    ...
    def need_validation(insight: JSONType) -> bool:
        if insight["type"] != PackagerCodeInsightImporter.get_type():
            raise ValueError(
                "insight must be of type "
                "{}".format(PackagerCodeInsightImporter.get_type())
            )
...
class CategoryImporter(InsightImporter):
    ...
    def process_product_insights(
        self, insights: Iterable[JSONType], automatic: bool
    ) -> Iterable[JSONType]:
        category_seen: Dict[str, Set[str]] = {}
        for t in (
            ProductInsight.select(
                ProductInsight.value_tag, ProductInsight.barcode
            ).where(ProductInsight.type == self.get_type())
        ).iterator():
