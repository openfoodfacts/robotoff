...
def crop_image_url(
    ...
    base_url = settings.BaseURLProvider.image_url(server_type, source_image)
...
class ProductInsight(BaseModel):
    ...
    # robotoff.types.InsightType.
    type = peewee.CharField(max_length=256, index=True)

    # Contains some additional data based on the type of the insight from
    # above.
    ...
    # Stores the timestamp of when this insight was annotated, either by a
    # human annotator or automatically.
    ...
    # Specifies the timestamp on an automatic_processing insight after which
    # the insight should be applied.
    process_after = peewee.DateTimeField(null=True)

    # Value tag contains the normalized 'value' of the insight based on the
    # OFF taxonomies.
    # Some insight types will populate both this field and the 'value' field
    # below, some only this field.
    value_tag = peewee.TextField(null=True, index=True)

    # Value stores the value of the insight, for example the brand name for a
    # product or the numeric value of the weight of the product.
    # Some insight types will populate both this field and the 'value_tag'
    # field above, some only this field.
    # This field is set for the following insight types: brand,
    # expiration_date, packager_code, packaging, product_weight and store.
    ...
    # Automatic processing is set on insights where Robotoff deems the
    # prediction to be valid, e.g. if the prediction is above a pre-determined
    # threshold.
    ...
    # This field refers to the number of unique IPs that have scanned this
    # product.
...
class AnnotationVote(BaseModel):
    ...
    # Currently logged in users do not need to vote on an insight for their
    ...
    # If the request has a device_id, use that - otherwise use a secure hash of
    # the IP address.
...
class ImagePrediction(BaseModel):
    ...
    They are created by API `ImagePredictorResource`,
    `ImagePredictionImporterResource` or cli `import_logos`
...
class LogoAnnotation(BaseModel):
    ...
    when we have a logo prediction above a certain threshold we create an
    entry, to ask user for annotation on the logo
    (https://hunger.openfoodfacts.org/logos) and eventual annotation will land
    there.
    """
    ...
    def get_server_type(self) -> ServerType:
        return ServerType[self.image_prediction.image.server_type]
